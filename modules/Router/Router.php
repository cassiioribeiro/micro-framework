...
class Router {
	...
	public function match() {
		...
		foreach($type as $pt => $func) {
			// Identifica os argumentos e substitui por regex (expressao regular)
			$pattern = preg_replace('(\{[a-z0-9]{0,}\})', '([a-z0-9]{0,})', $pt);

			// Faz o match de URL,ve se a URL bateu com a ER
			if(preg_match('#^('.$pattern.')*$#i', $url, $matches) === 1){
				array_shift($matches);//apagando o primeiro elemento
				array_shift($matches);
